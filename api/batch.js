...
async function analyzeCompact(token,item){
  ...
  const filtered=comparableFilter(raw,20,{title:q,query:q,sys:item.sys||"",upc}), listings=filtered.out, totals=listings.map(x=>x.total).filter(Number.isFinite), prices=listings.map(x=>x.price).filter(Number.isFinite), series=totals.length?totals:prices;
